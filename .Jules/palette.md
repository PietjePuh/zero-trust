...
## 2026-06-29 - Waiting for attached DOM elements in Playwright
**Learning:** When using Playwright to verify DOM elements that might be initially hidden or dynamically rendered (such as modal close buttons), using the default `wait_for_selector` can cause a TimeoutError because it waits for the element to be visible.
**Action:** Use `wait_for_selector('...', state='attached')` to ensure the element is found in the DOM without timing out waiting for strict visual visibility.
