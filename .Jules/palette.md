...
## 2024-05-24 - Escape Key Support for Search Inputs
**Learning:** When implementing Escape key to clear search inputs programmatically, it is crucial to dispatch a synthetic `input` event (`e.target.dispatchEvent(new Event('input'))`) after clearing the value. Without this, standard framework or vanilla JS event listeners relying on the `input` event won't trigger, leaving UI state (like filtered lists or "clear" button visibility) out of sync with the input's actual value. Additionally, Playwright tests can fail silently if there's a JavaScript crash during event processing, so checking for page errors via `page.on('pageerror')` is necessary.
**Action:** Always dispatch `new Event('input')` when clearing input fields programmatically and ensure tests look out for JavaScript errors.
