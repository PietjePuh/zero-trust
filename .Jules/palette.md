...
## 2026-06-14 - ARIA labels and titles for Icon-Only Buttons
**Learning:** Icon-only close and remove buttons (e.g., using `&times;` or `✕` symbols) require both an `aria-label` for screen reader accessibility and a `title` attribute for mouse hover discoverability.
**Action:** When creating or updating interactive elements that rely solely on iconography, always add explicit `aria-label` and `title` attributes simultaneously to guarantee accessibility and context for all users.
