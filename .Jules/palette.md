...
## 2026-06-03 - Accessible Tooltips for Modal Close Buttons\n**Learning:** Icon-only modal close buttons (like the cheat-sheet 'x') often lack `aria-label` and `title`, making them inaccessible to screen readers and difficult to interpret for mouse users who expect a hover tooltip. This applies equally to dynamically generated buttons like favorites.\n**Action:** When adding or auditing icon-only buttons (especially close controls and favorites toggles), always implement both `aria-label` (for screen readers) and `title` (for mouse users) attributes.
