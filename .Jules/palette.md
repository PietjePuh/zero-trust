...
## 2026-03-27 - Input Field Escape Key Handling
**Learning:** Search inputs often provide visual "clear" buttons for mouse users, but keyboard users expect the `Escape` key to serve the same function. Without it, users must manually backspace to clear queries.
**Action:** Always add `keydown` event listeners to search/filter inputs that listen for the `Escape` key, clear the value, trigger the filter update function, and ensure focus remains on the input field.
