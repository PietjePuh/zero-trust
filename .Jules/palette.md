...
## 2026-04-10 - Escape Key for Search Clearing
**Learning:** Search inputs often require manual deletion to clear, which can be tedious. Adding keyboard support to quickly clear the input improves accessibility and power-user experience.
**Action:** When implementing search inputs, bind the `Escape` key to clear the field, trigger the respective filtering function, and return focus to the input.
