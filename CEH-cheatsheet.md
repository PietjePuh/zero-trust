...
# Post-Exploitation & C2 (Command & Control)
**Modern Paradigm:** Move away from "Trojans" to "C2 Frameworks" that use beaconing, obfuscation, and legitimate protocols (HTTPS/DNS) for persistence.

## C2 Frameworks (MITRE T1105)
| Tool | Description | Type |
| --- | --- | --- |
| Cobalt Strike | Industry standard for adversary simulation. | Commercial |
| Sliver | Cross-platform (Go), implant-based C2. | Open Source |
| Mythic | Collaborative, multi-platform C2 framework. | Open Source |
| Covenant | .NET C2 framework focusing on offensive C# development. | Open Source |

## Persistence Tactics (MITRE TA0003)
| Technique | Description |
| --- | --- |
| Scheduled Tasks | Creating tasks via `schtasks` or `cron`. |
| Registry Run Keys | Adding entries to `HKCU\Software\Microsoft\Windows\CurrentVersion\Run`. |
| DLL Hijacking | Placing malicious DLLs in application search paths. |
| Service Creation | Creating new system services for SYSTEM-level execution. |

## Evasion (MITRE TA0005)
| Technique | Description |
| --- | --- |
| Process Injection | Injecting code into legitimate processes (e.g., `svchost.exe`). |
| AMSI Bypass | Bypassing Anti-Malware Scan Interface in Windows/PowerShell. |
| Living off the Land | Using legitimate tools (LOLBAS) like `certutil` or `bitsadmin`. |
